...
class IG_Pb_Core {
	...
	function custom_meta_boxes() {
		...
		if ( $this->check_support( 'has_editor' ) ) {
			// Element List
			include IG_PB_TPL_PATH . '/select-elements.php';
		}
	}
...
//				$this->register_sub_el( $class, 1 );
	...
	function shortcode_iframe_preview() {
		...
		if ( isset( $_GET['ig_shortcode_preview'] ) ) {
			...
			$html  = '<div id="shortcode_inner_wrapper" class="jsn-bootstrap3"><fieldset><legend>' . __( 'Preview', IGPBL ) . '</legend>';
			$html .= $content;
			$html .= '</fieldset></div>';
	...
	function hook_after_editor() {
		if ( $this->check_support() ) {
			...
			echo '<div class="tab-pane" id="ig_editor_tab2" style="display:none">'
					. '<div id="ig_before_pagebuilder">'
						. '<div class="jsn-section-content jsn-style-light" id="form-design-content">'
							. '<div class="ig-pb-form-container jsn-layout"></div>'
						. '</div>'
					. '</div>'
				. '</div>';
		}
