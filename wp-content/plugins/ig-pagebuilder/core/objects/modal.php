...
if ( ! class_exists( 'IG_Pb_Objects_Modal' ) ) {
	...
	class IG_Pb_Objects_Modal {
		 ...
		 * @return string
		 */
		static function get_shortcode_modal_settings( $settings, $shortcode = '', $input_params = null, $raw_shortcode = null, $has_preview = false ) {
			$i    = 0;
		...
		static function setting_tab_html( $shortcode, $tabs, $contents, $general_actions, $settings, $actions, $has_preview = false ) {
			...
			if ( $has_preview == true ) {
				$output = "<div class='col-md-6'>{$output}</div>";
			}
