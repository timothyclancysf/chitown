...
if ( ! empty( $shortcode ) ) {
	...
	<div id="ig-element-<?php echo esc_attr( IG_Pb_Helper_Shortcode::shortcode_name( $shortcode ) ); ?>">
		<div class="ig-pb-form-container jsn-bootstrap3">
			<div id="modalOptions" class="form <?php echo esc_attr( $submodal ); ?>">
	<?php
	if ( ! empty( $params ) ) {
	...
	if ( $el_type == 'element' ) {
		...
		if ( class_exists( $class ) ) {
			...
			if ( ! empty( $params ) ) {
				...
				IG_Pb_Helper_Shortcode::generate_shortcode_params( $instance->items, NULL, $extract_params, TRUE, FALSE, $extract_title, $has_preview );
			...
			$settings_html .= IG_Pb_Objects_Modal::get_shortcode_modal_settings( $settings, $shortcode, $extract_params, $params, $has_preview );
			
			// Add preview
			if ( $has_preview ) {
				$settings_html .= IG_Pb_Helper_Shortcode::render_parameter( 'preview' );
			}
