(function($) {
	...
	$.HandleElement.editElement = function() {
		...
		$('body').on("click", ".ig-dialog .ig-link-header", function (e) {
			e.stopPropagation();
		});
...
//						modal.close();
						...
						// Check is in pagebuilder layout
						var pb_layout = active_item.closest('.ig-pb-form-container').first();
						var is_in_layout = false;
						if ( pb_layout.hasClass('jsn-layout') ) {
							is_in_layout = true;
						}

						// Update Element Title to Active element (only for not child element)
						if ( ( !$.options.new_sub_element && append_title_el ) || is_in_layout == true) {
					...
					$.ajax({
						...
						complete: function(data, status) {
							if (status == 'success') {
								...
								/*modal.container.closest('.ui-dialog').find('.ui-dialog-titlebar').append(
										"<a class='ig-link-header' href='http://www.innogears.com/' target='_blank'>Support</a>"
										+ "<span class='ig-link-header'>|</span>"
										+ "<a class='ig-link-header' href='http://www.innogears.com/' target='_blank'>Report bug</a>");*/
		...
		$('.switchmode-button', switcher_group).on('click', function (e, doit){
			...
			if($(this).attr('id')	== 'status-off'){
				...
				if(cf){
					...
					$('#ig-pb-activity-undo').addClass('disabled');
					$('#ig-pb-activity-redo').addClass('disabled');
				...
				// Disable redo/undo buttons
				$('#ig-pb-activity-undo').removeClass('disabled');
				$('#ig-pb-activity-redo').removeClass('disabled');
